...
<div class="card shadow mb-4">
    ...
    <div class="card-header py-3">
        <h3 class="m-0 font-weight-bold text-primary">@lang('sentence.manage-drives')</h3>
    </div>
    <!-- Button Add  -->
    <div>
        <div class=" p-3 pt-4">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <!-- Table  -->
            <table class="table table-bordered datatable-drive " width="100%" cellspacing="0">
                <thead>
                    <tr>
                        <th width="5%">@lang('sentence.drive-id')</th>
                        <th width="5%">@lang('sentence.drive-logo')</th>
                        <th width="35%">@lang('sentence.drive-name')</th>
                        <th width="10%">@lang('sentence.drive-actions')</th>
                    </tr>
                </thead>
            </table>
        </div>
            ...
            <form method="post" id="sample_form" class="" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="name">@lang('sentence.drive-name'):</label>
                        <input type="text" class="form-control" id="name" name="name" aria-describedby="nameHelp" placeholder="@lang('sentence.drive-name')">
                        <!-- <small id="nameHelp" class="form-text text-muted">Your information is safe with us.</small> -->
                    </div>
                    <!-- Logo Input and Preview-->
                    <div class="form-group">
                        <label for="logo">@lang('sentence.drive-logo'):</label>
...
<script type="text/javascript">
    $(document).ready(function() {
            ...
            columnDefs: [
                // Add img to table
                {
                    ...
                    render: function(a, b, row) {
                        let img = "{{ asset('uploads/drives/') }}" + "/" + row.logo
        ...
        $('#create_record').click(function() {
            ...
            $('#action').val('Add');
            $('#form_result').html('');
            $("#logo_preview").attr("src", "{{ asset('uploads/drives/') }}" + "/default.png");
        ...
        $('#sample_form').on('submit', function(event) {
            ...
            if ($('#action').val() == 'Add') {
                action_url = "{{ route('drives.store') }}";
            }

            if ($('#action').val() == 'Edit') {
                action_url = "{{ route('drives.update') }}";
            }
            // Ajax send form
            $.ajax({
                url: action_url,
                ...
                success: function(data) {
                    ...
                    if (data.success) {
                        ...
                        $('#sample_form')[0].reset();
                        $('.datatable-drive').DataTable().ajax.reload();
        ...
        $(document).on('click', '.edit', function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $("#logo_preview").attr("src", "{{ asset('uploads/drives/') }}" + '/' + data.result.logo);
                    $('#hidden_id').val(id);
        ...
        $('#ok_button').click(function() {
            var url = '{{ route("drives.deletebyid", ":id") }}';

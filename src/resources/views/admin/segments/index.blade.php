...
<div class="card shadow mb-4">
    ...
    <div class="card-header py-3">
        <h3 class="m-0 font-weight-bold text-primary">@lang('manage-segments')</h3>
    </div>
    <!-- Button Add  -->
    <div>
        <div class=" p-3 pt-4">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <!-- Table  -->
            <table class="table table-bordered datatable-segment " width="100%" cellspacing="0">
                <thead>
                    <tr>
                        <th width="5%">@lang('segment-id')</th>
                        <th width="5%">@lang('segment-logo')</th>
                        <th width="35%">@lang('segment-letter')</th>
                        <th width="35%">@lang('segment-name')</th>
                        <th width="10%">@lang('segment-actions')</th>
                    </tr>
                </thead>
            </table>
        </div>
            ...
            <form method="post" id="sample_form" class="" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="name">@lang('segment-name'):</label>
                        <input type="text" class="form-control" id="name" name="name" aria-describedby="nameHelp" placeholder="@lang('segment-name')">
                        <!-- <small id="nameHelp" class="form-text text-muted">Your information is safe with us.</small> -->
                    </div>
                    <div class="form-group">
                        <label for="letter">@lang('segment-letter'):</label>
                        <input type="text" class="form-control" id="letter" name="letter" aria-describedby="letterHelp" placeholder="@lang('segment-letter')">
                        <!-- <small id="nameHelp" class="form-text text-muted">Your information is safe with us.</small> -->
                    </div>
                    <!-- Logo Input and Preview-->
                    <div class="form-group">
                        <label for="logo">@lang('segment-logo'):</label>
...
<script type="text/javascript">
    $(document).ready(function() {
            ...
            columnDefs: [
                // Add img to table
                {
                    ...
                    render: function(a, b, row) {
                        let img = "{{ asset('uploads/segments/') }}" + "/" + row.logo
        ...
        $('#create_record').click(function() {
            ...
            $('#action').val('Add');
            $('#form_result').html('');
            $("#logo_preview").attr("src", "{{ asset('uploads/segments/') }}" + "/default.png");
        ...
        $('#sample_form').on('submit', function(event) {
            ...
            if ($('#action').val() == 'Add') {
                action_url = "{{ route('segments.store') }}";
            }

            if ($('#action').val() == 'Edit') {
                action_url = "{{ route('segments.update') }}";
            }
            // Ajax send form
            $.ajax({
                url: action_url,
                ...
                success: function(data) {
                    ...
                    if (data.success) {
                        ...
                        $('#sample_form')[0].reset();
                        $('.datatable-segment').DataTable().ajax.reload();
        ...
        $(document).on('click', '.edit', function() {
            var id = $(this).attr('id');
            $('#form_result').html('');
            var url = '{{ route("segments.edit", ":id") }}';
        ...
        $('#ok_button').click(function() {
            var url = '{{ route("segments.deletebyid", ":id") }}';
